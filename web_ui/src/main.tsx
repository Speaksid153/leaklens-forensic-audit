...
function Results({ result, sourceName, csv, config }: { result: AuditResult; sourceName: string; csv: string; config: AuditConfig }) {
  ...
  const [candidateFailed, setCandidateFailed] = React.useState(false);
  ...
  const candidateUrl = React.useMemo(() => candidateContent ? URL.createObjectURL(new Blob([candidateContent], { type: "text/csv" })) : "", [candidateContent]);
  React.useEffect(() => {
    let active = true;
    setReportContent(""); setReportFailed(false); setCandidateContent(""); setCandidateFailed(false);
    ...
    pythonRuntime.buildCandidateCsv(csv, result).then((candidate) => { if (active) setCandidateContent(candidate); }).catch(() => { if (active) setCandidateFailed(true); });
    return () => { active = false; };
  ...
  React.useEffect(() => () => { if (candidateUrl) URL.revokeObjectURL(candidateUrl); }, [candidateUrl]);
